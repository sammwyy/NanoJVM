...
int jmevm_native_register(
    ...
    jmevm_native_fn fn)
{
    if (class_name == NULL || method_name == NULL || descriptor == NULL || fn == NULL) {
        return -1;
    }
...
static const struct jmevm_native_method *native_lookup_exact(
    ...
    size_t desc_len)
{
    ...
    return native_lookup_exact(class_bytes, class_len, method_bytes, method_len, desc_bytes, desc_len);
}

static int32_t native_println_int(
    jmevm_vm *vm,
    int32_t receiver,
    const int32_t *args,
    uint16_t argc)
