...
static jvm_object objects[MAX_OBJECTS];
static int heap_initialized = 0;

void jvm_heap_init(size_t size) {
  (void)size;
  for (int i = 0; i < MAX_OBJECTS; i++) {
    objects[i].type = JVM_OBJ_FREE;
  }
  heap_initialized = 1;
}

int jvm_heap_is_initialized(void) { return heap_initialized; }

jvm_object *jvm_heap_get_objects(void) { return objects; }
uint16_t jvm_heap_get_max_objects(void) { return MAX_OBJECTS; }

int32_t jvm_heap_alloc(const struct jvm_classfile *cf) {
  uint16_t obj_ref = 0;
  for (uint16_t i = 1; i < MAX_OBJECTS; i++) {
    if (objects[i].type == JVM_OBJ_FREE) {
      obj_ref = i;
      break;
    }
  }
  ...
  jvm_object *obj = &objects[obj_ref];
  obj->type = JVM_OBJ_CLASS;
  obj->cf = cf;
  obj->marked = 0;
...
int32_t jvm_heap_alloc_array(jvm_obj_type type, int32_t length) {
  ...
  size_t elem_size = (type == JVM_OBJ_ARRAY_INT || type == JVM_OBJ_ARRAY_OBJ)
                         ? sizeof(int32_t)
                         : 1;
...
int32_t jvm_array_length(int32_t array_ref) {
  if (array_ref <= 0 || array_ref >= MAX_OBJECTS)
    return 0;
  if (objects[array_ref].type == JVM_OBJ_FREE)
    return 0;
...
int32_t jvm_array_load_int(int32_t array_ref, int32_t index) {
  if (array_ref <= 0 || array_ref >= MAX_OBJECTS)
    return 0;
  jvm_object *obj = &objects[array_ref];
  if ((obj->type != JVM_OBJ_ARRAY_INT && obj->type != JVM_OBJ_ARRAY_OBJ) ||
...
int8_t jvm_array_load_byte(int32_t array_ref, int32_t index) {
  if (array_ref <= 0 || array_ref >= MAX_OBJECTS)
    return 0;
  jvm_object *obj = &objects[array_ref];
  if (obj->type != JVM_OBJ_ARRAY_BYTE || index < 0 || index >= obj->length)
    return 0;
...
void jvm_object_put_field(int32_t obj_ref, uint16_t field_index,
                          int32_t value) {
  if (obj_ref <= 0 || obj_ref >= MAX_OBJECTS) {
    return;
  }
...
const struct jvm_classfile *jvm_heap_get_classfile(int32_t obj_ref) {
  if (obj_ref <= 0 || obj_ref >= MAX_OBJECTS)
    return NULL;
  if (objects[obj_ref].type == JVM_OBJ_FREE)
    return NULL;
