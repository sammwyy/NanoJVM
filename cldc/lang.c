...
void jmevm_cldc_init(jmevm_vm *vm) {
  ...
  cf = jmevm_classfile_load_from_buffer(java_lang_Object_class,
                                        java_lang_Object_class_len);
  if (cf)
    jmevm_vm_register_class(vm, cf);

  cf = jmevm_classfile_load_from_buffer(java_lang_Throwable_class,
                                        java_lang_Throwable_class_len);
  if (cf)
    jmevm_vm_register_class(vm, cf);

  cf = jmevm_classfile_load_from_buffer(java_lang_Exception_class,
                                        java_lang_Exception_class_len);
  if (cf)
    jmevm_vm_register_class(vm, cf);

  cf = jmevm_classfile_load_from_buffer(java_lang_RuntimeException_class,
                                        java_lang_RuntimeException_class_len);
  if (cf)
    jmevm_vm_register_class(vm, cf);

  cf = jmevm_classfile_load_from_buffer(
      java_lang_NullPointerException_class,
  ...
  cf = jmevm_classfile_load_from_buffer(java_lang_System_class,
                                        java_lang_System_class_len);
  if (cf)
    jmevm_vm_register_class(vm, cf);

  cf = jmevm_classfile_load_from_buffer(java_lang_String_class,
