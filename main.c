...
static int read_file_bytes(const char *path, uint8_t **out_buf,
                           size_t *out_len) {
...
int main(int argc, char **argv) {
  if (argc < 2) {
    fprintf(stderr, "Usage: %s <Main.class> [Other.class ...]\n",
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (read_file_bytes(argv[i], &class_buf, &class_len) != 0) {
      fprintf(stderr, "Failed to read class file: %s\n", argv[i]);
      continue;
    }

    jmevm_classfile *cf =
        jmevm_classfile_load_from_buffer(class_buf, class_len);
    if (cf == NULL) {
      fprintf(stderr, "Failed to parse class file: %s\n", argv[i]);
      free(class_buf);
      continue;
    }
  ...
  if (main_cf == NULL) {
    fprintf(stderr, "No valid main class loaded\n");
    jmevm_vm_destroy(vm);
    return 1;
  }
  ...
  if (jmevm_classfile_extract_main(main_cf, &m) != 0) {
    fprintf(stderr, "Main class does not have a main method\n");
    jmevm_vm_destroy(vm);
    return 1;
  }
